...
- 最后，构建出的统计量$$\frac{\sum\left(\hat{y}_{i}-\bar{y}\right)^{2}/1}{\sum\left(y_{i}-\hat{y}_{i}\right)^{2}/n-2}=\frac{SSR/1}{SSE/(n-2)}=\frac{MSR}{MSE}$$，它恰好符合F分布（F分布啥来着来着？是nY/mZ，Y和Z都是$$\chi$$卡方分布，而卡方分布又是正态分布的平方和的分布；观察这个式子，F分布可能就是如此得出的，我猜）。
...
首先，教材上说，$$\hat{\beta}_1$$服从正态分布，标准差为$$\sigma_{\hat{\beta1}_1} = \frac{\sigma}{\sqrt{\sum x^2_i - \frac{1}{n}(\sum x_i)^2}}$$，$$\sigma$$是误差项$$\epsilon$$的标准差，这个不得而知，所有用它对应的估计量$$s_e$$近似替代，得到估计量的$$\hat{\beta}_1$$的方差$$s_{\hat{\beta}_1}$$。
