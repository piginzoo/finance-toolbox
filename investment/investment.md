...
这里再详细说一下，一元回归$$\beta$$的细节：

$$E(r_{it})=r_f + \beta_i (E(r_{Mt} - r_f) + \epsilon_{it})$$

这个式子，没法做回归，变形一下，

$$E(r_{it}) -r_f = \alpha_i + \beta_i (E(r_{Mt} - r_f) + \epsilon_{it})$$

说一下下标：t表示时间周期，比如每日、每月或者每年，用它来表示一个时间序列；i是一个固定的值，用来代指某一个资产。

好了，可以做回归了，你可以拿到市场基准利率，一般用一年期国债利率$$r_f$$;你也可以得到资产的投资收益，是一个时间序列，每个时间周期上的收益$$r_{it}$$；你也可以得到市场的收益，它也是个时间序列，比如股指的每个时间周期的收益率$$r_{Mt}$$，然后你就可以做一元回归了。

你一定可以得到一堆的，每个时间周期上的残差，$$\epsilon_{it}$$，它的期望应该是0。你还可以得到一个$$\alpha_i$$，理论上，他应该接近于0，如果你想验证CAPM模型，它就应该为0。如果是你想找出这个资产，可以有一个超额收益，那么这个$$\alpah$$就是超额收益。
...
？？？为何说ATP是相对定价？相对是什么含义？为何CAPM就叫做绝对定价？绝对在哪里？
徐高老师解释说，CAPM叫“绝对”的原因是，它是从无倒有，从10个假设中，直接推导出资产的价格，所以叫绝对。

APT不是这个思路，他是参考着别人的价格来定我的价格，一个市场，就存在一个价格，体现了一价定律，那么最终一个资产价格会收敛到一个稳定价格上，且，是参照别人的价格制定的。
徐高老师给的例子，就是，汉堡、可乐、汉堡可乐套餐，这三个价格，可以相互参照，相互作用，如果有价格的不合理，就可以通过其他的套利，导致价格回归到合理位置。

>一价定律或单一价格法则指出：“假设其他条件不变，在有效率市场的情况下，任何同质物品的价值应该等同。”

所以，我们再来理解一下**“相对”**的意思，后面会讲，就是通过组合几个资产，通过调整不同的权重，“发现”定价不合理的资产，对他进行套利，从而逼迫他，重新回到合理的价格上。你丫相对不合理，那我就通过套利组合，逼迫你回到相对合理的价格上。

除了用一个相对定价的方式来定价的思路外，这里再说一个为何要引入APT定价的原因。

CAPM给了一个定价模型，按理说，如果它靠谱，你把市场的资产，去做一个CAPM检验，比如用一个资产（如股票、基金）的收益率，去跑一下CAPM模型，结果你发现，很多时候，不一定符合，徐高老师说，特别是中国的股票，根本没法拟合出来，这也好理解，CAPM的那些假设，我们大A股，大多不满足。所以，这就逼着搞量化的人们，去想别的办法来给资产定价啦，这个也是APT由来的原因。

所以，就引入了APT，APT是基于因子模型。而因子模型，其实就是用一些所谓的“因子”，来线性表达出资产价格来；有了因子模型，就可以自圆其说地推导出来APT，APT就是说，你看，因为你们这些资产可以表达成因子的线性组合，然后我就依据这个线性组合，就可以推呀推，推导出，你们的价格是可以通过套利，被逼迫达到一个稳态价格，从而让套利空间消失，这个动态过程，最终定出了资产的合理价格。

反过来，因为有ATP的严谨证明，就会反过来，让你找各种因子，变得有意义，否则，就是一个暴力数据挖掘，找出这种相关性而已。有了APT理论，就可以让你觉得，噢！我找出的这些因子，真的是可以影响和决定定价的呀。

再多说一句，即使挖掘出一些因子，但是，也不是这些因子就是靠谱的、可信的，可以放心大胆地去用来做投资的依据，还是要不断地通过市场去验证和修正，保证其的真实有效性。

另外一个问题，为何多因子模型，是一个线性模型，为何不能是一个2次的，甚至其他非线性模型？
徐高老师说，这个就是通过APT给出的（APT实际给出了线性的依据）

>"市场没有套利机会的时候，一些受到共同因子共同影响的资产的期望回报率，应该满足线性关系"(线性就是这么来的！)

APT并没有指出到底是哪些因子？狠泛化的一个理论，约束很小，狠灵活。

latent factor。

徐高老师说，实际上可以找到很多很多因子，对股票收益率的拟合度都很好，甚至$$R^2$$可以到0.9，很高了，但是，如果你用它去预测真正的未来的收益率，$$R^2$$会降到甚至0.02。但是，他没有解释为什么？不太理解...
