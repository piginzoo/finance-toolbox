...
不同于CAPM那么强的、那么多的假设，因子模型，不对市场做那么多假设，只是假设它是有一个线性的模型来构建：

$$r_{it}=a_{i}+b_{i 1} F_{1t}+b_{i 2} F_{2t}+\cdots+b_{i k} F_{kt}+e_{it}$$

其中个，$$F_i$$是因子，

过去CAPM中，只有市场收益$$r_M$$这一个，现在多因子可以构建多个了，但是，可能你构建的因子里还是不会包含一些敏感的因素，
这些没有被估计到因素，都被合并到了残差项$$e_{i}$$中了：

残差项的期望为0：$$E(e_i)=0$$

各因子之间，因子与误差项之间，误差项之间的协方差应该都为0：$$cov(F_{it_1},F_{it_2})=0, cov(F_i,e_i)=0,  cov(e_{it_1},e_{it_2})=0$$，

如果不独立，说明，残差项$$e_i$$中，还有些信息没有被提取出来，应该还可以再抽象出一个新的因子，这个模型还需要再挖掘。
嗯！这个方法好，可以用来做模型好坏的一个检验标准。

再看个细节，

上式中，都有个下标$$t$$，就是说，这个式子应该对任何一个时刻都满足。 
